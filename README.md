...
Logs are written to `~/.catp/audit/<agentId>/<YYYY-MM-DD>/actions.jsonl`. Each entry chains on the previous commitment hash, forming a tamper-evident sequence. Layer 2 anchors these to on-chain Poseidon commitments verifiable via the deployed `Halo2Verifier.sol`.
...
| 2 | `ProveAuthorization` Halo2 circuit — in-circuit Poseidon, 3 public inputs, k=12 | ✅ Complete (11 tests) |
| 2 | WASM prover bundle (`catp-circuits/wasm`) | ✅ Complete — `compute_policy_commitment` / `prove_authorization` / `verify_authorization` |
| 2 | `AgentAuthorizer.sol` + `ActionData.sol` | ✅ Complete (16 tests) |
| 2 | `Halo2Verifier.sol` — k=12, 3 public inputs, shared KZG SRS | ✅ Complete |
...
| 2 | TypeScript SDK — `PolicyBuilder`, `AuthorizerClient`, `ProofClient` | ✅ Complete (29 tests) |
